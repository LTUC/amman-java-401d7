...
public class App {
    ...
    private static List<StreamsWithFunctionalPrograming> setup(){
        List<StreamsWithFunctionalPrograming> greatDevs= new ArrayList<>();
        greatDevs.add(new StreamsWithFunctionalPrograming("Ed", true, 0, 0, new String[0], false, "skydiving", true));
        greatDevs.add(new StreamsWithFunctionalPrograming("Matt", true, 0, 0, new String[0], true, "video games", true));
        greatDevs.add(new StreamsWithFunctionalPrograming("Jen", false, 1, 0, new String[]{ "Fang"}, false, "snowboarding", true));
        greatDevs.add(new StreamsWithFunctionalPrograming("Haustin", true, 2, 1, new String[]{"Bob Ross", "Lenny Kravitz"}, false, "video games", true));
        greatDevs.add(new StreamsWithFunctionalPrograming("Patrick", false, 2, 18, new String[]{"Robin", "Batman"}, false, "guitarist", true));
    ...
    public static void main(String[] args) {
        ...
        List<StreamsWithFunctionalPrograming> people = setup();
        // ***** TEST CASE 1 *****
        ...
        for (StreamsWithFunctionalPrograming person : people)
        {
        ...
        // Method 1
        //List<StreamsWithFunctionalPrograming> tiredPeople = people.stream().filter(s -> s.isTired == true).collect(toList());
        // findFirst() and findAny() are basically equivalent for our purposes
        StreamsWithFunctionalPrograming tiredPerson = people.stream().filter(s -> s.isTired == true).findAny()
                .orElse(null);
        ...
        List<StreamsWithFunctionalPrograming> tiredPeople = new ArrayList<>();
        List<StreamsWithFunctionalPrograming> notTiredPeople = new ArrayList<>();
        ...
        List<StreamsWithFunctionalPrograming> sortedPeople = new ArrayList<>(people);
        ...
        // This version uses a method reference: StreamsWithFunctionalPrograming::getName
        String allPeopleSeparatedBySemicolonsFunctional = people.stream().map(StreamsWithFunctionalPrograming::getName).collect(joining(";"));
